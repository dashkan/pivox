...
static const CropTemplate FREE_TEMPLATE { "Free", std::nullopt };
    ...
    : maxHistory_(options.maxHistory)
{
    ...
    allTemplates.push_back(FREE_TEMPLATE);
    for (auto& t : options.templates) {
        if (t.label != FREE_TEMPLATE.label) {
            allTemplates.push_back(t);
        }
    }

    auto defaultTemplate = options.defaultTemplate.value_or(FREE_TEMPLATE);
...
void ImageEditorEngine::applyTemplate(const CropTemplate& tmpl) {
    ...
    auto [cropW, cropH] = applyCropTemplate(
        *tmpl.ratio, state_.cropWidth, state_.cropHeight,
...
void ImageEditorEngine::zoomIn() {
    state_.zoom = std::min(zoom_constants::kZoomMax, state_.zoom + zoom_constants::kZoomStep);
    state_.zoomMode = ZoomMode::Manual;
    if (onChangeCallback_) onChangeCallback_(state_);
}

void ImageEditorEngine::zoomOut() {
    state_.zoom = std::max(zoom_constants::kZoomMin, state_.zoom - zoom_constants::kZoomStep);
    state_.zoomMode = ZoomMode::Manual;
    if (onChangeCallback_) onChangeCallback_(state_);
}
...
void ImageEditorEngine::setZoom(double level) {
    state_.zoom = std::max(zoom_constants::kZoomMin, std::min(zoom_constants::kZoomMax, level));
    state_.zoomMode = ZoomMode::Manual;
    if (onChangeCallback_) onChangeCallback_(state_);
}
...
void ImageEditorEngine::onPointerDown(double cropX, double cropY, bool isAltOrMiddle) {
    ...
    if (isAltOrMiddle && state_.zoomMode == ZoomMode::Manual && state_.zoom > 100) {
        panOrigin_ = PanOrigin{ cropX, cropY, state_.panOffset.x, state_.panOffset.y };
    ...
    auto handle = hitTestHandles(cropX, cropY, state_.cropWidth, state_.cropHeight, hitRadius);
...
void ImageEditorEngine::onPointerMove(double cropX, double cropY,
    ...
    if (dragOrigin_.has_value()) {
        ...
        if (dragOrigin_->handle == DragHandle::Move) {
            ...
            auto ratio = orig.activeTemplate ? orig.activeTemplate->ratio : std::nullopt;
            if (ratio.has_value()) {
...
std::optional<DragHandle> ImageEditorEngine::hitTest(double cropX, double cropY) const {
    double hitRadius = render_constants::kHandleHitRadius / viewportScale_;
    return hitTestHandles(cropX, cropY, state_.cropWidth, state_.cropHeight, hitRadius);
}
...
bool ImageEditorEngine::isEditStateDirty(const EditState& a, const EditState& b) {
    return a != b;
}
