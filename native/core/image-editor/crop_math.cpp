...
double computeMinScaleWithPerspective(double cropW, double cropH, double imgW,
  ...
  // Per-axis focal length: separate multipliers let us match Photos.app
  // intensity.
...
TranslationBounds computeTranslationBounds(double cropW, double cropH,
  ...
  double txLimit1 = absCos > kEps
                        ? (hIW - (hCW * absCos + hCH * absSin)) / absCos
                        : std::numeric_limits<double>::infinity();
  double txLimit2 = absSin > kEps
                        ? (hIH - (hCW * absSin + hCH * absCos)) / absSin
                        : std::numeric_limits<double>::infinity();
  double maxTx = std::max(0.0, std::min(txLimit1, txLimit2));

  double tyLimit1 = absSin > kEps
                        ? (hIW - (hCW * absCos + hCH * absSin)) / absSin
                        : std::numeric_limits<double>::infinity();
  double tyLimit2 = absCos > kEps
                        ? (hIH - (hCW * absSin + hCH * absCos)) / absCos
                        : std::numeric_limits<double>::infinity();
...
bool isCropSizeValid(double newCropW, double newCropH, double imgW, double imgH,
                     double scale, double angleRad, double perspVRad,
                     double perspHRad) {
...
std::optional<DragHandle> hitTestHandles(double px, double py, double cropW,
  ...
  HandlePos handles[] = {
      {DragHandle::NW, -hw, -hh}, {DragHandle::N, 0, -hh},
      {DragHandle::NE, hw, -hh},  {DragHandle::W, -hw, 0},
      {DragHandle::E, hw, 0},     {DragHandle::SW, -hw, hh},
      {DragHandle::S, 0, hh},     {DragHandle::SE, hw, hh},
  };
