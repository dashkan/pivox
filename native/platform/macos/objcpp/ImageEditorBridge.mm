...
- (void)toggleFlipHorizontal { _engine->toggleFlipHorizontal(); }
- (void)toggleFlipVertical { _engine->toggleFlipVertical(); }
...
- (void)zoomIn { _engine->zoomIn(); }
- (void)zoomOut { _engine->zoomOut(); }
- (void)zoomToFit { _engine->zoomToFit(); }
- (void)setZoom:(double)level { _engine->setZoom(level); }
- (void)enterCropMode { _engine->enterCropMode(); }
- (void)exitCropMode { _engine->exitCropMode(); }
...
- (nullable NSString*)hitTestX:(double)x y:(double)y {
    ...
    switch (*result) {
        case pivox::DragHandle::NW: return @"nw";
        case pivox::DragHandle::N:  return @"n";
        case pivox::DragHandle::NE: return @"ne";
        case pivox::DragHandle::W:  return @"w";
        case pivox::DragHandle::E:  return @"e";
        case pivox::DragHandle::SW: return @"sw";
        case pivox::DragHandle::S:  return @"s";
        case pivox::DragHandle::SE: return @"se";
        case pivox::DragHandle::Move: return @"move";
    }
}
...
- (IEBState*)currentState {
    ...
    if (s.activeTemplate.has_value()) {
        ...
        state.activeTemplate = t.ratio.has_value()
            ? [[IEBCropTemplate alloc] initWithLabel:@(t.label.c_str()) ratio:*t.ratio]
            : [IEBCropTemplate freeform];
    }

    NSMutableArray* templates = [NSMutableArray arrayWithCapacity:s.templates.size()];
