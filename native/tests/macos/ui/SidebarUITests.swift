...
class SidebarUITests: XCTestCase {
  ...
  func testSidebarPositionIsStableAfterResize() throws {
    ...
    let operatorText = app.staticTexts.matching(identifier: "Operator").firstMatch
    XCTAssertTrue(operatorText.waitForExistence(timeout: 5))

    takeScreenshot(named: "sidebar-before-resize")
    let initialFrame = operatorText.frame
    ...
    let afterFrame = operatorText.frame
    print("After resize Operator frame: \(afterFrame)")

    let yDrift = abs(afterFrame.minY - initialFrame.minY)
    print("Y drift: \(yDrift)")
    XCTAssertLessThan(
      yDrift, 10, "Sidebar items should not jump vertically after resize (drifted \(yDrift)pt)")
  }
}
