...
class AuthUITests: XCTestCase {
  ...
  private func registerAccount(
    email: String? = nil, password: String = "Testpass123!",
    ...
    let operatorText = app.staticTexts["Operator"]
    XCTAssertTrue(
      operatorText.waitForExistence(timeout: 10),
  ...
  func testEmailFieldIsFocusedOnLaunch() throws {
    ...
    XCTAssertEqual(
      emailField.value as? String, "test",
  ...
  func testAccessibilityIdentifiersExist() throws {
    ...
    XCTAssertTrue(
      app.checkBoxes["login-remember-me"].exists || app.toggles["login-remember-me"].exists)
  ...
  func testSignInWithNonExistentAccount() throws {
    ...
    XCTAssertTrue(
      errorText.waitForExistence(timeout: 15),
  ...
  func testRegisterSwitchToLogin() throws {
    goToRegister()
    let link =
      app.links["register-switch-login"].exists
      ? app.links["register-switch-login"]
      : app.buttons["register-switch-login"]
    XCTAssertTrue(link.waitForExistence(timeout: 5))
    link.click()
    XCTAssertTrue(
      app.textFields["login-email"].waitForExistence(timeout: 3),
  ...
  func testRememberMePreFillsEmailAfterSuccessfulSignIn() throws {
    ...
    let rememberMe =
      app.checkBoxes["login-remember-me"].exists
  ...
  func testRememberMeDoesNotSaveOnFailedLogin() throws {
    ...
    XCTAssertTrue(freshEmail.waitForExistence(timeout: 5))
    XCTAssertNotEqual(
      freshEmail.value as? String, "shouldnot@besaved.com",
