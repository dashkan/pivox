...
TEST_F(ImageEditorEngineTest, Apply16x9Template) {
  ...
  EXPECT_NEAR(engine.state().cropWidth / engine.state().cropHeight, 16.0 / 9.0,
              0.001);
}
...
TEST_F(ImageEditorEngineTest, HistoryRespectesMaxDepth) {
  ImageEditorEngine smallHistory(
      ImageEditorEngine::Options{{}, std::nullopt, 3});
...
TEST(ImageEditorEngineStandaloneTest, CustomTemplatesPreserved) {
  ...
  opts.templates = {CropTemplate{"16:9", 16.0 / 9.0},
                    CropTemplate{"4:3", 4.0 / 3.0}};
