...
// RoleGrants is the same role-to-permission grant data as `matrix`,
// in ordered-slice form keyed by role name. It seeds the
// role_permissions table (per-org system-role grants) at org
// bootstrap and in the dev seed, so permission checks can also
// resolve in SQL — e.g. the membership-scoped operations list —
// without N+1 Has() round-trips. Kept in lockstep with `matrix` by
// TestRoleGrants_MatchMatrix.
