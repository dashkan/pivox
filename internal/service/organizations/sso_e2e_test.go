...
// SsoConfig is now a Pivox-side row only — Keycloak owns the upstream
// OIDC/SAML provider, so UpdateSsoConfig no longer provisions anything
// upstream. These tests pin the row-CRUD contract at the RPC boundary.
...
// it, and Get returns the persisted state both times.
