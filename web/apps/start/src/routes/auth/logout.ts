 ...
 */
/**
 * The POST /auth/logout handler, extracted from the route so it can be unit
 * tested as a plain `(ctx) => Promise<Response>` function — no router runtime,
 * no reaching into the framework's route object. The route below just wires it
 * to the server method table.
 */
export async function handleLogout({ request }: { request: Request }): Promise<Response> {
