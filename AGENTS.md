...
cloud API + Worker Process, an envoy ingress container, the ngrok
tunnel, and the TanStack Start dev server. It supersedes the
...
npm run aspire:build          # tsc typecheck of apphost.mts
npm run aspire:lint           # eslint apphost.mts
```

**Always** `aspire:build` + `aspire:lint` after editing `apphost.mts`.
Use `aspire logs`/`aspire describe` to diagnose — don't guess.

Conventions and gotchas (each was paid for once; don't relearn them):

- **`apphost.mts` is the source of truth.** `.aspire/modules/` (the
  generated TS API), `.data/`, and `node_modules/` are gitignored and
  regenerated by the Aspire CLI. Never edit `.aspire/modules/`. New
  integrations are added with `aspire add <name>` (first-party
  `Aspire.Hosting.*` packages get TS builders; CommunityToolkit
  packages are C#-only and won't surface in the TS API).
- **Go apps (`addGoApp`) and the Vite app run as host processes**, not
  ...
  Keycloak, envoy, and ngrok are containers.
...
- **`PIVOX_DATABASE_URL` must be a libpq URL** (`postgres://…`), not
  ...
  Built in `apphost.mts` from the Postgres endpoint + parameters.
...
- **Container env is NOT inherited from direnv `.envrc`** (host
  ...
  `withEnvironment` (e.g. `NGROK_AUTHTOKEN` from `process.env`).
